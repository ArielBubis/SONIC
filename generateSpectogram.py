...
def optimize_audio_visualization(audio_dir, output_dir, fig_size, max_workers=None, file_extension='.mp3'):
    """
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=max_workers) as executor:
        ...
        visualization_tasks = [
            ...
            (generate_chromagram, (audio_dir, visualization_dirs['chromagrams'], fig_size)),
            # (generate_fft_spectrum, (audio_dir, visualization_dirs['fft_spectra'], fig_size)),
            (generate_tempogram, (audio_dir, visualization_dirs['tempograms'], fig_size))
        ]
...
def generate_mel_spectrogram(audio_dir, output_dir, fig_size, n_fft, n_mels, gain_factor=1.0, vmin=-80, vmax=0):
    """
...
def generate_chromagram(audio_dir, output_dir, fig_size):
    """
    ...
    for filename in audio_files:
        ...
        fig, ax = plt.subplots(figsize=fig_size)
        librosa.display.specshow(
            chroma, x_axis='time', y_axis='chroma', cmap="coolwarm", ax=ax)
        ax.axis('off')
        plt.subplots_adjust(left=0, right=1, top=1, bottom=0)  # Remove padding and margins
        output_filepath = os.path.join(output_dir, f"{os.path.splitext(filename)[0]}.png")
        utils.save_image_with_cv2(fig, output_filepath)
...
def generate_tempogram(audio_dir, output_dir, fig_size):
    """
...
# def generate_fft_spectrum(audio_dir, output_dir, fig_size):
#     """
