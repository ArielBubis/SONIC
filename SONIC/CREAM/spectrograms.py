...
#             spectrogram, display_func = generate_mel_spectrogram(y, sr, hop_length)
#             display_kwargs = {
#                 'x_axis': 'time', 'y_axis': 'mel', 'cmap': 'magma', 'vmin': DB_MIN, 'vmax': DB_MAX
#             }
            
#         elif spectrogram_type == 'chroma':
#             logging.info(f"Generating chromagram for {audio_dir}")
#             spectrogram, display_func = generate_chromagram(y, sr, hop_length)
...
#             spectrogram, display_func = generate_tempogram(y, sr, hop_length)
...
#         logging.info(f"Saved {spectrogram_type} spectrogram to {output_filepath}")
#     except Exception as e:
#         logging.error(f"Error generating {spectrogram_type} spectrogram for file {audio_dir}: {e}")

# def generate_mel_spectrogram(y, sr, hop_length):
#     try:
#         n_fft = 2048
#         n_mels = CNN_SIZE
#         gain_factor = 1.0
...
#         return spectrogram, display_func
#     except Exception as e:
#         logging.error(f"Error generating mel spectrogram: {e}")
#     return None, None

# def generate_chromagram(y, sr, hop_length):
#     try:
...
# def generate_tempogram(y, sr, hop_length):
#     try:
...
#         io.save_image(fig, output_filepath)
